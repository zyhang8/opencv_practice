...
int main()
{
	...
	*/
	...
	//进行霍夫线变换
	Mat hough_src, hough_img;
	Mat gray_src, gray_img;
	vector<Vec2f> lines_src;//定义一个矢量结构lines用于存放得到的线段矢量集合
	vector<Vec2f> lines_img;
	cvtColor(canny_src, gray_src, CV_GRAY2BGR);//转化边缘检测后的图为灰度图
	cvtColor(canny_img, gray_img, CV_GRAY2BGR);//转化边缘检测后的图为灰度图

	HoughLines(canny_src, lines_src, 1, CV_PI / 150, 100, 0, 0);
	HoughLines(canny_img, lines_img, 1, CV_PI / 150, 100, 0, 0);
