...
int main()
{
	...
	Mat carHSV, carImage[3];
	cvtColor(img, carHSV, COLOR_BGR2HSV);
	split(carHSV, carImage);
	imshow("HSV", carHSV);
	...
	{
		const uchar* data_h = carImage[0].ptr<uchar>(j);
		const uchar* data_s = carImage[1].ptr<uchar>(j);
		const uchar* data_v = carImage[2].ptr<uchar>(j);
