...
int main()
{
	...
	//高斯
	namedWindow("gaosi_h");
	createTrackbar("内核值:", "高斯滤波", &g_nGaussianBlurValue, 50, on_GaussianBulr_H);
	on_GaussianBulr_H(g_nGaussianBlurValue, 0);
	imshow("高斯滤波", gaosi);

	imshow("S", Image[1]);
	namedWindow("gaosi_s");
	createTrackbar("内核值:", "高斯滤波", &g_nGaussianBlurValue, 50, on_GaussianBulr_S);
	on_GaussianBulr_S(g_nGaussianBlurValue, 0);
	imshow("高斯滤波", gaosi);

	imshow("V", Image[2]);
	namedWindow("gaosi_v");
	createTrackbar("内核值:", "高斯滤波", &g_nGaussianBlurValue, 50, on_GaussianBulr_V);
