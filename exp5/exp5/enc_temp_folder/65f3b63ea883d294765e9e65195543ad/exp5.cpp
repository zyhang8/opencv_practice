...
Mat range,canny_out, graysrc,dstt;
...
Mat getContours(int, void*) {
	...
	for (int i = 0; i < contours.size(); i++)
	{
		...
		circle(range, Point(box[i].center.x, box[i].center.y), 5, Scalar(0, 255, 0), -1, 8);  //绘制最小外接矩形的中心点
		box[i].points(rect);  //把最小外接矩形四个端点复制给rect数组
		rectangle(range, Point(boundRect[i].x, boundRect[i].y), Point(boundRect[i].x + boundRect[i].width, boundRect[i].y + boundRect[i].height), Scalar(0, 255, 0), 2, 8);
		for (int j = 0; j < 4; j++)
		{
			line(range, rect[j], rect[(j + 1) % 4], Scalar(0, 0, 255), 2, 8);  //绘制最小外接矩形每条边
		}
	}
	imshow("rect_range", range);
	/*
...
int main()
{
	...
	cvtColor(range, graysrc, CV_RGB2BGRA);
